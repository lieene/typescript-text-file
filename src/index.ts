  ...
  export class TextEdit
  {
    ...
    insertLine(...lines: [number, string][]): void
    {
      lines.forEach(l =>
      {
        ...
        let range = lineRg !== undefined ? lineRg.startAnchor :
          line === 0 ? new L.Range(0, 0) : new L.Range(this.text.source.length, 0);
        if (!string.endsWith('\n')) { string = string + '\r\n'; }
    ...
    remove(...spans: (L.Range | Span)[]): void
    {
      spans.forEach(range =>
      {
        ...
        {
          range = this.text.fullRange.intersection(range as L.Range);
        }
        this.edits.pushOrdered({ range, replace: '' }, compareEdit);
      });
    }
