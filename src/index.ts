...
export class Text implements Text.ReadonlyText
{
  ...
  constructor(source: any, sourceType?: 'Path' | 'Source')
  {
    ...
    if (sourceType === 'Path')
    {
      ...
      {
        throw new Error('@types/node package not found');
      }
      try
      {
  ...
  get AsReadonly(): Text.ReadonlyText
  {
    return this;
  }
  ...
  get lineCount()
  {
    return this.lineAnchors.length - 1;
  }
  ...
  *eachline()
  ...
  slice(arg0: number | Text.Span | L.Range, arg1?: number): string
  {
    ...
    {
      return this.source.slice(arg0.start, arg0.end);
    }
  }
  ...
  convert(pos: Text.Pos | number | L.Range | Text.Span): Text.Pos | number | L.Range | Text.Span
  {
    ...
    if (L.IsNumber(pos))
    {
      ...
      if (pos <= 0)
      {
        return new Text.Pos(1, 1);
      }
      let anchors = this.lineAnchors;
      let anchorIdx = anchors.binarySearch(pos);
      if (anchorIdx >= 0)
      {
        return new Text.Pos(anchorIdx + 1, 1);
      }
      anchorIdx = ~anchorIdx;
    ...
    else if (Text.IsPos(pos))
    {
      ...
      if (pos.line >= this.lineAnchors.length)
      {
  ...
  protected applyEditSync(text: Text, editCallback: (editor: Text.TextEdit) => void, callback: (e: Error, r: Text) => void): void
  {
    ...
    try
    {
      ...
      for (let i = 0, len = edits.length; i < len; i++)
      {
        ...
        if (endOfEdit < preEnd)
        {
          newText = oldText.slice(endOfEdit, preEnd) + newText;
        }
        if (e.replace.length > 0)
        {
          newText = e.replace + newText;
        }
        preEnd = e.range.start;
      }
      if (preEnd > 0)
      {
        newText = oldText.slice(0, preEnd) + newText;
      }
...
export namespace Text
{
  ...
  export function IsPos<T>(p: Pos | T): p is Pos
  {
    return Object.getPrototypeOf(p) === Pos.prototype;
  }
  ...
  export class Span
  {
    ...
    constructor(arg0: L.Range | Pos | [number, number] | number, arg1: Text | Pos | [number, number] | number, arg2?: Text)
    {
      ...
      {
        let [start, end, t] = L.IsNumber(arg0) ?
          [arg0, arg1 as number, arg2 as Text] :
          [arg0.start, arg0.end, arg1 as Text];
  ...
  export function IsSpan<T>(s: Span | T): s is Span
  {
    return Object.getPrototypeOf(s) === Span.prototype;
  }
  ...
  export class TextEdit
  {
    ...
    insert(...args: (number | Pos | string | [number | Pos, string])[]): void
    {
      ...
      if (L.IsString(text))
      {
        ...
        } else
        {
          ...
          {
            pos = Math.min(this.text.source.length, pos);
          }
        }
    ...
    removeLine(...lines: number[])
    {
      lines.forEach(l =>
      {
        let range = this.text.lineRange(l, "include line end");
        if (range !== undefined)
        { this.edits.pushOrdered({ range, replace: '' }); }
      });
    }
    remove(...spans: (L.Range | Span)[]): void
    {
      spans.forEach(range =>
      {
        if (Text.IsSpan(range))
        {
          range = this.text.convert(range);
        } else
        {
          let intersec = this.text.fullRange.intersection(range);
          range = intersec === L.Range.Relation.Before ? new L.Range(this.text.fullRange.end, 0) : intersec === L.Range.Relation.After ? new L.Range(this.text.fullRange.start, 0) : intersec;
        }
